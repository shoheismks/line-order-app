...
function doPost(e) {
  try {
    ...
    orderSheet.appendRow([
      ...
      payload.note || "",
      calculateServerTotal_(items, customerId),
      "未処理",
      "",
      "",
      "",
...
function refreshCustomerUrls_() {
  ...
  const customers = customerSheet.getRange(2, 1, lastRow - 1, 5).getValues()
    .filter(row => row[0]);
...
function setupWorkbook_() {
  ...
  getOrCreateSheet_(ss, SHEET_CUSTOMER_MASTER, [
    ["顧客ID", "顧客名", "LINEユーザーID", "ステータス", "備考"],
    ["CUST-A", "A商店", "", "有効", "デモ顧客"],
    ["CUST-B", "Bレストラン", "", "有効", "価格・表示商品の違い確認用"],
    ["CUST-C", "Cカフェ", "", "停止", "停止顧客デモ"]
  ]);
 ...
 * サーバー側で合計金額を再計算
 */
function calculateServerTotal_(items, customerId) {
  const customerProductMap = getCustomerProductMap_(customerId);
...
function syncOrderStatusToDetails_() {
  ...
  const orderValues = orderSheet.getRange(2, 1, orderLastRow - 1, 17).getValues();
  const statusMap = {};
...
function setDashboardFormulas_() {
  ...
  sh.getRange("D2").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,"未処理")`);
  sh.getRange("B3").setFormula(`=COUNTIFS('${SHEET_ORDERS}'!B2:B,">="&TODAY(),'${SHEET_ORDERS}'!B2:B,"<"&TODAY()+1)`);
  sh.getRange("D3").setFormula(`=SUMIFS('${SHEET_ORDERS}'!I2:I,'${SHEET_ORDERS}'!B2:B,">="&EOMONTH(TODAY(),-1)+1,'${SHEET_ORDERS}'!B2:B,"<"&EOMONTH(TODAY(),0)+1)`);
  sh.getRange("B4").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,"出荷待ち")`);
  sh.getRange("D4").setFormula(`=COUNTIF('${SHEET_ORDERS}'!P2:P,"一部欠品")+COUNTIF('${SHEET_ORDERS}'!O2:O,"欠品")`);
  ...
  sh.getRange("B10").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,A10)`);
  sh.getRange("B11").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,A11)`);
  sh.getRange("B12").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,A12)`);
  sh.getRange("B13").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,A13)`);
  sh.getRange("B14").setFormula(`=COUNTIF('${SHEET_ORDERS}'!J2:J,A14)`);
}
...
function setValidations_() {
  ...
  orderSheet.getRange("J2:J1000").setDataValidation(statusRule);
  orderSheet.getRange("P2:P1000").setDataValidation(stockCheckRule);
...
function formatCustomerMaster_(sh) {
  ...
  const widths = [120,180,220,100,260];
  widths.forEach((w, i) => sh.setColumnWidth(i + 1, w));
...
function formatOrders_(sh) {
  ...
  sh.getRange("A:Q").setVerticalAlignment("middle");
  ...
  sh.getRange("L:L").setNumberFormat("yyyy-mm-dd");
  sh.getRange("Q:Q").setNumberFormat("yyyy-mm-dd hh:mm");

  const widths = [160,140,180,110,140,110,90,220,110,100,100,110,120,150,220,110,140];
  widths.forEach((w, i) => sh.setColumnWidth(i + 1, w));

  sh.getRange("A1:Q1000").setBorder(true, true, true, true, true, true, "#D9E2EC", SpreadsheetApp.BorderStyle.SOLID);
  sh.getRange("A2:Q1000").setWrap(true);
...
function applyOrderConditionalFormatting_(sh) {
  const range = sh.getRange("A2:Q1000");
  const rules = [];

  rules.push(SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied('=$J2="未処理"')
    .setBackground("#FFF2CC")
    .setRanges([range])
    .build());

  rules.push(SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied('=$J2="確認済"')
    .setBackground("#E2F0D9")
    .setRanges([range])
    .build());

  rules.push(SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied('=$J2="出荷待ち"')
    .setBackground("#DDEBF7")
    .setRanges([range])
    .build());

  rules.push(SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied('=$J2="出荷済"')
    .setBackground("#E7E6E6")
    .setRanges([range])
    .build());

  rules.push(SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied('=$J2="キャンセル"')
  ...
  rules.push(SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied('=OR($P2="欠品",$P2="一部欠品")')
    .setFontColor("#C00000")
    .setBold(true)
    .setRanges([sh.getRange("P2:P1000")])
    .build());
