...
顧客別ページ内に `注文履歴 / Order history` ボタンを追加しています。
...
Apps Script側では `action=orders` APIを追加しています。

例:

```text
https://script.google.com/macros/s/xxxx/exec?action=orders&customerId=CUST-A
```

反映には `google-apps-script/Code_customer_order_history.gs` をApps Scriptに貼り付け、新バージョンで再デプロイしてください。
