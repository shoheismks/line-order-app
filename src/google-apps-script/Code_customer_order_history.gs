...
function doGet(e) {
  ...
  if (action === "orders") {
    try {
      const result = getOrderHistoryForCustomer_(customerId);
      const json = JSON.stringify({ ok: true, ...result });
 ...
 * 顧客別注文履歴を取得
 */
function getOrderHistoryForCustomer_(customerId) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const orderSheet = ss.getSheetByName(SHEET_ORDERS);
  const detailSheet = ss.getSheetByName(SHEET_DETAILS);
  const customer = getCustomerById_(customerId) || {
  ...
  const orderValues = orderSheet.getRange(2, 1, orderLastRow - 1, Math.min(orderSheet.getLastColumn(), 19)).getValues();

  const detailValues = detailSheet && detailSheet.getLastRow() >= 2
    ? detailSheet.getRange(2, 1, detailSheet.getLastRow() - 1, 11).getValues()
    : [];
  ...
  const orders = orderValues
    .filter(row => row[3] === customerId)
    .map(row => ({
      orderId: row[0],
      orderedAt: formatDateTimeForApi_(row[1]),
      lineUserId: row[2],
      customerId: row[3],
      customerName: row[4],
      deliveryDate: formatDateKey_(row[5]),
      deliveryTime: row[6],
      note: row[7],
      productSubtotal: Number(row[8] || 0),
      shippingFee: Number(row[9] || 0),
      totalAmount: Number(row[10] || 0),
      status: row[11] || "",
      personInCharge: row[12] || "",
      plannedShipDate: formatDateKey_(row[13]),
      deliveryMethod: row[14] || "",
      trackingNumber: row[15] || "",
      internalMemo: row[16] || "",
      stockCheck: row[17] || "",
      updatedAt: formatDateTimeForApi_(row[18]),
      items: detailMap[row[0]] || []
    }))
